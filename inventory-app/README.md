...
- 記録の一覧（月別・ラベル付きチップ表示）・編集・削除・CSVエクスポート（UTF-8 BOM付き）
- ダブルチェック運用: 記録ごとに「未確認／1回目完了／ダブルチェック完了」を管理し、ダブルチェック実施者と日時を記録。一覧上部に進捗サマリを表示
...
| 棚卸し記録 | `RecordTable` | アプリからの入力が1行ずつ追記される（19列） |
